...
class MCPManagementScreen(MDScreen):
    ...
    def _build_ui(self):
        ...
        servers_card.add_widget(MDLabel(
    ...
    def _update_servers_ui(self, servers):
        ...
        if not servers:
            self.servers_list.add_widget(MDLabel(
        ...
        for server in servers:
            ...
            icon = IconLeftWidget(
                ...
                icon_color=[0.2, 0.8, 0.2, 1] if server['connected'] else [0.8, 0.2, 0.2, 1],  # Green or Red
            )
    ...
    def _update_tools_ui(self, tools):
        ...
        if not tools:
            self.tools_list.add_widget(MDLabel(
    ...
    def add_websocket_server(self, name: str, url: str, ssl_verify: bool = True):
        ...
        if parsed_url.scheme in ['wss', 'https']:
            ssl_config = {
                "verify": ssl_verify,
                "allow_self_signed": not ssl_verify
            }
    ...
    async def _add_websocket_server_async(self, name: str, url: str, ssl_config: dict = None):
        ...
        try:
            success = await self.chat_manager.add_mcp_server(name, server_url=url, ssl_config=ssl_config)
