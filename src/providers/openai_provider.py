    ...
    **Future Compatibility**:
    ...
    async def complete(
        ...
        **Quality Optimization**:
        ...
        try:
            ...
            raise AuthenticationError(f"Invalid OpenAI API key. Check OPENAI_API_KEY.") from e
        except RateLimitError as e:
            logger.warning(f"OpenAI rate limit exceeded: {e}")
            raise RateLimitError(f"OpenAI rate limit exceeded. Try again later.") from e
    ...
    async def stream_complete(
        ...
        **Error Recovery**:
        ...
        try:
            ...
            logger.error(f"OpenAI stream authentication error")
            raise AuthenticationError(f"Invalid OpenAI API key for streaming") from e
        except RateLimitError as e:
            logger.warning(f"OpenAI stream rate limit: {e}")
            raise RateLimitError(f"Streaming rate limit exceeded") from e
        except APITimeoutError as e:
    ...
    async def health_check(self) -> bool:
        ...
        **Service Issues**:
        ...
        """
        try:
            logger.debug(f"Performing health check for OpenAI API")
            ...
            if "authentication" in str(e).lower():
                logger.error(f"OpenAI health check failed: Authentication error")
            elif "rate limit" in str(e).lower():
                logger.warning(f"OpenAI health check failed: Rate limited")
            elif "network" in str(e).lower() or "connection" in str(e).lower():
                logger.warning(f"OpenAI health check failed: Network issue")
            else:
